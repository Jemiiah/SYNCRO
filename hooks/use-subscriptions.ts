...
export function useSubscriptions({
    ...
    const handleAddSubscription = useCallback(
        ...
        [subscriptions, maxSubscriptions, emailAccounts, updateSubscriptions, undo, onToast, onUpgradePlan]
    );
        ...
        async (id: number, updates: any) => {
            try {
                ...
                const updatedSubs = subscriptions.map((sub: any) => {
                    ...
                    const editedFields = Object.keys(updates).filter((key: string) => updates[key as keyof typeof updates] !== (sub as any)[key]);

                    return {
                        ...sub,
                        ...updates,
                        manually_edited: true,
                        edited_fields: [...new Set([...(sub.edited_fields || sub.editedFields || []), ...editedFields])],
