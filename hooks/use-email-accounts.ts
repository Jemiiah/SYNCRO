...
export function useEmailAccounts({
        ...
        (emailAccountData: any) => {
            ...
            setIntegrations(
                integrations.map((int) =>
                    int.name === "Gmail" ? { ...int, accounts: emailAccounts.length + 1 } : int
                )
            );
            onToast({
    ...
    const handleRemoveEmailAccount = useCallback(
        (id: number) => {
            ...
            const affectedSubscriptions = subscriptions.filter((sub: any) => (sub as any).emailAccountId === id || sub.email_account_id === id);
        ...
        [emailAccounts, subscriptions, updateSubscriptions, addToHistory, integrations]
    );
    ...
    const handleRescanEmail = useCallback((id: number) => {
        setEmailAccounts(
            emailAccounts.map((acc) => (acc.id === id ? { ...acc, lastScanned: new Date() } : acc))
        );
    }, [emailAccounts]);
